...
do
    ...
    function HOUND.DB.getRadarData(typeName)
        ...
        local data = UTILS.DeepCopy(HOUND.DB.Radars[typeName])
    ...
    function HOUND.DB.getPlatformData(DcsObject)
        ...
        local platformData={
            pos = UTILS.DeepCopy(DcsObject:getPosition().p),
            isStatic = false,
            isAerial = false,
        }
...
do
    ...
    function HOUND.Utils.PointClusterTilt(points,MagNorth,refPos)
        ...
        for _,point in pairs(points) do
            ...
            if biasVector == nil then biasVector = V else biasVector = UTILS.Vec2Add(biasVector,V) end
        end
        --UTILS.PrintTableToLog(biasVector,indent,noprint)
        return (l_math.atan2(biasVector.y,biasVector.x) + magVar) % pi_2
    end
    ...
    function HOUND.Utils.getRoundedElevationFt(elev,resolution)
        ...
        return HOUND.Utils.roundToNearest(UTILS.MetersToFeet(elev),resolution)
    end

    function HOUND.Utils.roundToNearest(input,nearest)
        return UTILS.Round(input/nearest) * nearest
    end
    ...
    function HOUND.Utils.DecToDMS(cood)
        ...
        return {
            ...
            m = minutes,
            s = sec,
            mDec = UTILS.Round(dec ,3),
            sDec = UTILS.Round((UTILS.Round(dec,3)*1000)-(minutes*1000))
        }
    end
    ...
    function HOUND.Utils.getBR(src,dst)
        ...
        local BR = {}
        local dir = UTILS.GetDirectionRadians(UTILS.VecSubtract(dst,src),src) -- pass src to get magvar included
        BR.brg = UTILS.Round(UTILS.ToDegree( dir ))
        BR.brStr = string.format("%03d",BR.brg)
        BR.rng = UTILS.Round(UTILS.MetersToNM(UTILS.VecDist2D(dst,src)))
        return BR
    end
    ...
    function HOUND.Utils.getFormationCallsign(player,override,flightMember)
        ...
        local shortcallsign = player:GetCallsign() or "unknown91" -- e.g.Uzi91, but we want Uzi 9 1
        ...
        local formationCallsign = callsignroot
        callsign =  formationCallsign .. " " .. callnumbermajor -- player.callsign[2]
        if flightMember then
            callsign = callsign .. " " .. callnumberminor -- player.callsign[3]
        end
    ...
    function HOUND.Utils.Geo.checkLOS(pos0,pos1)
        ...
        local dist = UTILS.VecDist2D(pos0,pos1)
    ...
    function HOUND.Utils.TTS.getTtsTime(timestamp)
        ...
        local DHMS = UTILS.GetDHMS(timestamp)
    ...
    function HOUND.Utils.TTS.getVerbalContactAge(timestamp,isSimple,NATO)
        ...
        local DHMS = UTILS.GetDHMS(ageSeconds)
    ...
    function HOUND.Utils.Elint.getAzimuth(src, dst, sensorPrecision)
        ...
        local vec = UTILS.VecSubtract(dst, src)
    ...
    function HOUND.Utils.Elint.getSignalStrength(src, dst, maxDetection)
        ...
        local dist = UTILS.VecDist3D(src,dst)
                ...
                for _,drawObject in pairs(drawLayer["objects"]) do
                    if drawObject["name"] == zoneName and drawObject["primitiveType"] == "Polygon" then
                        ...
                        if drawObject["polygonMode"] == "free" and HOUND.Length(drawObject["points"]) >2 then
                            points = UTILS.DeepCopy(drawObject["points"])
                            table.remove(points)
                        end
    ...
    function HOUND.Utils.Zone.getGroupRoute(GroupName)
        if type(GroupName) == "string" and Group.getByName(GroupName) then
            local points = UTILS.DeepCopy( _DATABASE:GetGroupTemplate( GroupName ).route.points )
            local newpoints = {}
            for point_num, point in pairs(points) do
            if not point.point then
              newpoints[point_num] = { x = point.x, y = point.y }
            else
              newpoints[point_num] = point.point --it's possible that the ME could move to the point = Vec2 notation.
              end
            end
            return newpoints
        end
    ...
    function HOUND.Utils.Polygon.threatOnSector(polygon,point, radius)
        if type(polygon) ~= "table" or HOUND.Length(polygon) < 3 or not HOUND.Utils.Dcs.isPoint(UTILS.Vec2toVec3(polygon[1])) then
            return
        end
        if not HOUND.Utils.Dcs.isPoint(point) then
            return
        end
        local inPolygon = UTILS.IsPointInPolygon(point,polygon)
    ...
    function HOUND.Utils.Polygon.filterPointsByPolygon(points,polygon)
        ...
        for _,point in pairs(points) do
            if UTILS.IsPointInPolygon(point,polygon) then
    ...
    function HOUND.Utils.Polygon.circumcirclePoints(points)
        ...
        local clonedPoints = UTILS.DeepCopy(points)
    ...
    function HOUND.Utils.Polygon.clipOrHull(polyA,polyB)
        ...
        if Polygon == nil then
            local points = UTILS.DeepCopy(polyA)
            for _,point in pairs(polyB) do
                table.insert(points,UTILS.DeepCopy(point))
            end
    ...
    function HOUND.Utils.Polygon.azMinMax(poly,refPos)
        if not HOUND.Utils.Dcs.isPoint(refPos) or type(poly) ~= "table" or HOUND.Length(poly) < 2 or UTILS.IsPointInPolygon(refPos,poly) then
            return
        end

        local points = UTILS.DeepCopy(poly)
        for _,pt in pairs(points) do
            pt.refAz = UTILS.GetDirectionRadians(UTILS.VecSubtract(pt,refPos))
        end
    ...
    function HOUND.Utils.Cluster.weightedMean(origPoints,initPos,threashold,maxIttr)
        ...
        local points = HOUND.Utils.Geo.setHeight(UTILS.DeepCopy(origPoints))
        if HOUND.Length(points) == 1 then return UTILS.DeepCopy(points[1]) end
        ...
        if not HOUND.Utils.Dcs.isPoint(current_mean) then
            current_mean = HOUND.Utils.getAvgPoint(origPoints)
        end
        ...
        while not converged do
            last_mean = UTILS.DeepCopy(current_mean)
            local totalDist = 0
            local totalInvWeight = 0
            for _,point in pairs(points) do
                point.dist = UTILS.VecDist2D(last_mean,point)
            ...
            for _,point in pairs(points) do
                ...
                current_mean = UTILS.VecAdd(current_mean,UTILS.ScalarMult(UTILS.VecSubstract(point,current_mean),weight))
            end
            ittr = ittr + 1
            converged = UTILS.VecDist2D(last_mean,current_mean) < threashold or ittr == maxIttr
        end
        HOUND.Utils.Geo.setHeight(current_mean)
        return UTILS.DeepCopy(current_mean)
    end

    function HOUND.Utils.Cluster.getDeltaSubsetPercent(Table,referencePos,NthPercentile,returnRelative)
        local t = UTILS.DeepCopy(Table)
        local len_t = HOUND.Length(t)
        t = HOUND.Utils.Geo.setHeight(t)
        if not referencePos then
            referencePos = HOUND.Utils.getAvgPoint(t)
        end
        for _,pt in ipairs(t) do
            pt.dist = UTILS.VecDist3D(referencePos,pt)
        end
        ...
        if returnRelative then
            for i = 1,#returnTable do
                returnTable[i] = UTILS.VecSubstract(returnTable[i],referencePos)
            end
        end
    ...
    function HOUND.Utils.Filter.groupsByPrefix(prefix)
        ...
        local groups = {}
        for groupName, groupData in pairs(_DATABASE.GROUPS) do
    ...
    function HOUND.Utils.Filter.staticObjectsByPrefix(prefix)
        if type(prefix) ~= "string" then return {} end
        local objects = {}
        for objectName, unit in pairs(_DATABASE.STATICS) do
    ...
    function HOUND.Utils.getAvgPoint(points)
      local avgX, avgY, avgZ, totNum = 0, 0, 0, 0
      for i = 1, #points do
            --log:warn(points[i])
            local nPoint = UTILS.Vec2toVec3(points[i])
        if nPoint.z then
          avgX = avgX + nPoint.x
          avgY = avgY + nPoint.y
          avgZ = avgZ + nPoint.z
        ...
        for i = 1,numSteps do
            ...
            if HoundUtils.Dcs.isPoint(point) and HoundUtils.Dcs.isPoint(self:getPos()) then
                ...
                elseif i == numSteps/2 then
                    ellipse.major = point
                    ellipse.majorCG = UTILS.VecDist2D(self:getPos(),point)
                elseif i == 3*(numSteps/4) then
                    if HoundUtils.Dcs.isPoint(ellipse.minor) then
                        ellipse.minor = UTILS.VecDist2D(ellipse.minor,point)
                    end
                elseif i == numSteps then
                    if HoundUtils.Dcs.isPoint(ellipse.major) then
                        ellipse.major = UTILS.VecDist2D(ellipse.major,point)
                        ellipse.majorCG = ellipse.majorCG / (ellipse.majorCG + UTILS.VecDist2D(self:getPos(),point))
                    end
                end
            end
        end
...
do
    ...
    function HOUND.Contact.Emitter.calculateEllipse(estimatedPositions,refPos,giftWrapped)
        ...
        local a = UTILS.Round(l_math.abs(min.x)+l_math.abs(max.x))
        local b = UTILS.Round(l_math.abs(min.y)+l_math.abs(max.y))
        ...
        uncertenty_data.az = UTILS.Round(l_math.deg(uncertenty_data.theta))
    ...
    function HOUND.Contact.Emitter:processData()
        if self:getPreBriefed() then
            if type(self.DcsObject) == "table" and type(self.DcsObject.isExist) == "function" and self.DcsObject:isExist()
                then
                    ...
                    if UTILS.VecDist2D(unitPos.p,self.pos.p) < 0.25 then return end
            ...
            if HOUND.Length(platformDatapoints) > 0 then
                for _,datapoint in pairs(platformDatapoints) do
                    ...
                    if HoundUtils.Dcs.isPoint(datapoint:getPos()) then
                        local point = UTILS.DeepCopy(datapoint:getPos())
        ...
        if HOUND.Length(estimatePositions) > 2 or (HOUND.Length(estimatePositions) > 0 and staticPlatformsOnly) then
            ...
            self.uncertenty_data.az = UTILS.Round(l_math.deg((self.uncertenty_data.theta+UTILS.GetNorthCorrection(self.pos.p)+pi_2)%pi_2))
    ...
    function HOUND.Contact.Emitter:useUnitPos(unitPosMarker)
        ...
        self.pos.p = UTILS.DeepCopy(unitPos.p)
        self:calculateExtrasPosData(self.pos)
...
do
    ...
    function HOUND.Contact.Site:updatePos()
        ...
        for _,emitter in ipairs(self.emitters) do
            if emitter:hasPos() then
                self.pos.p = UTILS.DeepCopy(emitter:getPos())
                break
            end
        end
    ...
    function HOUND.Contact.Site:ensurePrimaryHasPos(refPos)
        ...
        if ( not primary:hasPos() ) then
            for _,emitter in ipairs(self.emitters) do
                if ( emitter:hasPos() ) then
                    primary.pos = UTILS.DeepCopy(emitter.pos)
                    primary.uncertenty_data = UTILS.DeepCopy(emitter.uncertenty_data)
                    break
                end
            end
            ...
            if ( not primary:hasPos() and HoundUtils.Dcs.isPoint(refPos)) then
                ...
                primary.pos.p = UTILS.DeepCopy(refPos)
...
do
    ...
    function HOUND.Sector.checkIn(args,skipAck)
        ...
        for _,otherPlayer in pairs(gSelf:findGrpInPlayerList(player.groupId,_DATABASE.CLIENTS)) do
...
do
    ...
    function HOUND.Sector:createCheckIn()
        ...
        grpMenuDone = {}
        for _,player in pairs(_DATABASE.CLIENTS) do
            if player and player:IsAlive() then
            local grpId = player:GetGroup():GetID()
            local playerUnit = Unit.getByName(player:GetName())
...
do
    ...
    function HoundElint:onEvent(DcsEvent)
        ...
        if DcsEvent.id == world.event.S_EVENT_BIRTH
            ...
            and DcsEvent.initiator:getPlayerName() ~= nil
            and HOUND.setContains(_DATABASE.CLIENTS,DcsEvent.initiator:getName())
